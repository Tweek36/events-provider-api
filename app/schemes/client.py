...
class Place(BaseModel):
    ...
    changed_at: datetime = Field(..., description="Дата и время последнего изменения места в формате ISO 8601")
    created_at: datetime = Field(..., description="Дата и время создания места в формате ISO 8601")
    ...
    seats_pattern: str = Field(
        ...,
        description=r"Строка, описывающая схему рассадки в формате '\{секция\}\{начало\}-\{конец\},\{секция\}\{начало\}-\{конец\},...', например 'A1-10,B1-20'",
...
class Event(BaseModel):
    ...
    changed_at: datetime = Field(..., description="Дата и время последнего изменения события в формате ISO 8601")
    created_at: datetime = Field(..., description="Дата и время создания события в формате ISO 8601")
...
class SeatsResponse(BaseModel):
    seats: list[str] = Field(
        ...,
        description=r"Список доступных мест в формате '\{секция\}\{номер\}', например 'A1', 'B15' и т.д.",
    )
...
class RegisterRequest(BaseModel):
    ...
    seat: str = Field(
        ...,
        description=r"Выбранное место в формате '\{секция\}\{номер\}', например 'A1', 'B15' и т.д.",
        pattern=r"^[A-Z]\d+$",
    )
...
class RegisterResponse(BaseModel):
    ticket_id: uuid.UUID = Field(..., description="Уникальный идентификатор зарегистрированного билета")
...
class Metadata(BaseModel):
    ...
    last_changed_at: str | None = Field(None, description="Дата последнего изменения данных в формате 'YYYY-MM-DD'")
